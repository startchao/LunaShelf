...
test('main TTS lifecycle has no automatic foreground resume or retry path', async () => {
  ...
  assert.ok(playMethod.indexOf('resetSpeechEngine()') < playMethod.indexOf('speakNext(generation)'), 'Play must invalidate stale speech before creating the new utterance');
...
test('pause, stop, and background suspension invalidate before canceling speech', async () => {
  ...
  const reset = queue.slice(queue.indexOf('  resetSpeechEngine() {'), queue.indexOf('  play() {', queue.indexOf('  resetSpeechEngine() {')));
  ...
  assert.ok(reset.indexOf('playback.begin()') < reset.indexOf('speechSynthesis.cancel()'));
  assert.match(reset, /oldUtterance\.onend = null/);
  assert.match(reset, /oldUtterance\.onerror = null/);
  for (const method of [pause, stop, background]) assert.match(method, /resetSpeechEngine\(\)/);
